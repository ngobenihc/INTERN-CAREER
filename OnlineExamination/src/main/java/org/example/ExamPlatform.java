...
import static org.example.CreatePaper.createQuestionPaper;
...
public class ExamPlatform {
    ...
    static final List<Examination> exams = new ArrayList<>();
    static UserInput currentUser;

    public static void main(String[] args) {
        // Sample data for testing
        users.put("Lecture", new UserInput("Lecture", "lecture@123"));
        users.put("Student", new UserInput("Student", "student@123"));
        ...
        exams.get(1).addQuestion(new MultipleChoiceQuestion("Number of primitive data types in Java are?", List.of("8", "15", "14"), 1));
        exams.get(1).addQuestion(new MultipleChoiceQuestion("What is the size of float and double in java?", List.of("64 and 32", "32 and 64", "64 and 64"), 2));
        exams.get(1).addQuestion(new MultipleChoiceQuestion("Automatic type conversion is possible in which of the possible cases?", List.of("int to long", "int to bytes", "long to int"), 1));
        //exams.get(0).addQuestion(new );
